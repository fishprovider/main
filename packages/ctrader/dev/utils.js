...
const renewTokens = async (connection, refreshToken) => {
    ...
    await Mongo.collection('accounts').updateMany({
...
const getConfig = async () => {
    const account = await Mongo.collection('accounts').findOne({ _id: env.typeId }, {
        projection: {
            config: 1,
        },
    });
...
const createConnection = async (onEvent, configOverride) => {
    await start();
    ...
    const connection = new Connection(config, onEvent || (() => undefined), (err) => Logger.error('Connection error', err), () => undefined);
