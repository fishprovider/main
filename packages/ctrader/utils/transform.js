const transformLong = (value, moneyDigits = 0) => {
    const divide = 10 ** moneyDigits;
    return value.toNumber() / divide;
};
const transformOrder = (order) => {
    const { orderId, positionId, tradeData, utcLastUpdateTimestamp, } = order;
...
const transformPosition = (position) => {
    const { positionId, tradeData, commission, usedMargin, swap, moneyDigits, utcLastUpdateTimestamp, } = position;
    const { symbolId, volume, openTimestamp, } = tradeData;
    return {
        ...position,
        ...tradeData,
        positionId: positionId.toNumber().toString(),
...
const transformDeal = (deal) => {
    const { dealId, orderId, positionId, symbolId, volume, filledVolume, executionTimestamp, utcLastUpdateTimestamp, commission, moneyDigits, closePositionDetail, } = deal;
    return {
        ...deal,
        ...closePositionDetail,
...
const transformAccountInfo = (accountInfo) => {
    const { ctidTraderAccountId, balance, depositAssetId, moneyDigits, traderLogin, leverageInCents, } = accountInfo;
    return {
        ...accountInfo,
...
const transformBar = (bar) => {
    const { volume, low, deltaOpen, deltaClose, deltaHigh, utcTimestampInMinutes, } = bar;
