...
const getChannel = redisKeys.pendingOrders;

function useWatchPendingOrders(providerId: string) {
  ...
  useEffect(() => {
    if (socket) {
      ...
      Logger.debug('Skipped useWatchPendingOrders', providerId);
    }
    return () => {
      if (socket) {
        const channel = getChannel(providerId);
...
export default useWatchPendingOrders;
